...
<h3 class="titleServic">Formulario de Registro</h3>
		...
		$nombres = primeraMayuscula($nombres);
		$apellidos = primeraMayuscula($apellidos);
		
		$consulta = "INSERT INTO autor (nombre, apellido, cedula, facebook, twiter) VALUES ('$nombres', '$apellidos', '$cedula', '$facebook', '$twiter')";
		}
		$respueta = ejecutarConsulta($consulta, $link);
		if($respueta["success"] == true){
		echo "la consulta se ejecuto con exito </br>";
		echo "La consulta anterior afecto ".$respueta["numRows"]." filas";
		}else{
		$respueta["error"];
		}
		?>
